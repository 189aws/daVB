...
export ippz=${ippz:-''}
export warp=${warp:-''}
# 强制使用VPS本机IP做出口：出站走direct，不经过warp或CF中转
# 如需warp出口，可在脚本前手动设置 warp=xs 等变量
export _force_direct_out=yes
export name=${name:-''}
export oap=${oap:-''}
...
# 优选IP/域名池：CF anycast 全段IP + 知名优选域名 + 第三方优选节点，随机取一个作为入口
cfip_pool="
162.159.192.1
162.159.192.9
162.159.192.57
...
# 每次调用随机返回一个入口地址
cfip() {
  pool_list=$(echo "$cfip_pool" | grep -v '^$')
  total=$(echo "$pool_list" | wc -l)
  idx=$(( (RANDOM * RANDOM + RANDOM) % total + 1 ))
  echo "$pool_list" | sed -n "${idx}p"
}
if [ -e "$HOME/agsbx/xray" ]; then
...
echo "注：入口地址从CF优选IP池随机选取，可自行更换为其他优选IP或域名；如是回源端口需手动修改443或者80系端口"
...
if [ "$vlvm" = "Vmess" ]; then
vmatls_link1="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${sxname}vmess-ws-tls-argo-$hostname-443\", \"add\": \"$(cfip)\", \"port\": \"443\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"tls\", \"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"chrome\"}" | base64 -w0)"
...
vmatls_link6="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${sxname}vmess-ws-tls-argo-$hostname-2096\", \"add\": \"$(cfip)\", \"port\": \"2096\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"tls\", \"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"chrome\"}" | base64 -w0)"
echo "$vmatls_link6" >> "$HOME/agsbx/jh.txt"
vma_link7="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${sxname}vmess-ws-argo-$hostname-80\", \"add\": \"$(cfip)\", \"port\": \"80\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
...
vma_link13="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${sxname}vmess-ws-argo-$hostname-2095\", \"add\": \"$(cfip)\", \"port\": \"2095\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
